...
private final AchievementServiceImpl achievementService;
    ...
    public  ResponseEntity<Achievement> addAchievement(@Valid @RequestBody Achievement achievement){
        if (achievementService.existsByName(achievement.getName())) {
            System.out.println("name of the Achievenebt alreadyExists");
            // TODO kako da dodam u exception error ?
            return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
        }
        Achievement achievementsaved = achievementService.createAchievement(achievement);
        return new ResponseEntity<>(achievementsaved, HttpStatus.CREATED);
    }
